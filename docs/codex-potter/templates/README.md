...
本目录提供每轮八件套交接包中 6 份可直接复制的模板（计划/编码/测试/提交/结束/总结）。目标是把“可续跑”变成默认行为：每轮开始先立计划，每轮结束必有交接包。

模板文件：

- `plan-template.md`：本轮计划（含阅读清单、任务拆分、完成定义、验证计划）
- `code-template.md`：本轮编码/执行记录（含实际改动、偏离与阶段内验证）
- `test-template.md`：本轮测试记录（含环境、命令、结果、回归点）
- `commit-template.md`：本轮提交记录（含提交拆分、审阅重点、未提交状态）
- `close-template.md`：本轮结束记录（含索引更新、下一轮起点与结束检查）
...
- 任何人只要进入某个 round 目录，就能快速定位计划/验证/交接信息。
- 目录名天然可排序，方便回看历史演进。
...
2. 先创建 `round-overview.md` 和 `next-round-suggestions.md`，再复制模板补齐其余六份必备文档：
   - `plan.md` 从 `plan-template.md` 复制
   - `code.md` 从 `code-template.md` 复制
   - `test.md` 从 `test-template.md` 复制
   - `commit.md` 从 `commit-template.md` 复制
   - `close.md` 从 `close-template.md` 复制
   - `summary.md` 从 `summary-template.md` 复制
3. 先填 `plan.md` 的“阅读清单”和“范围/非目标”，再开始派发子代理任务。
4. 实际落地过程与偏离说明写入 `code.md`；所有验证命令与关键输出摘要写入 `test.md`。
5. 结束本轮前填写 `commit.md`、`close.md` 与 `summary.md`，并回链到 `round-overview.md` 与 `next-round-suggestions.md`。
6. 保留模板自带的“上层入口”区块；`round-overview.md` 与 `next-round-suggestions.md` 也要手工写入同样的回链，避免新 round 重新出现孤页。
