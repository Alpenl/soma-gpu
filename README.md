...
如果想直接复现当前三类固定切片上最稳的 structure-only 候选，可把 preset 换成 `real-mcp-chunk48ov8-deltapose4`；它会在同一组 corrected baseline 参数上把 chunk 结构切到 `runtime.sequence_chunk_size=48`、`runtime.sequence_chunk_overlap=8`，并额外叠加 `runtime.sequence_delta_pose=4`。当前 slice matrix 复核显示：它相对 plain `chunk48ov8` 保住了 fast-turn / dirty 的 seam 改善，同时把 stable / fast-turn / dirty 三段的 `reference_stageii_elapsed_delta_s` 都拉回到负值，并进一步压低三段 `chunk_seam_pose`；剩余未完全收回的缺口主要只剩 stable `chunk_seam_transl` 的轻微正 delta，因此它更适合作为当前 segment sweep 的 structure-only 候选，而不是直接替换默认 full-run candidate。
...
- 如果当前轮次是在固定三类切片上继续追 structure-only 主线，可显式加 `--candidate-preset real-mcp-chunk48ov8-deltapose4`；这样会直接切到当前最稳的 `chunk48/8 + delta_pose=4` 结构候选，而不需要再手写 `--candidate-cfg runtime.sequence_chunk_size=48 --candidate-cfg runtime.sequence_chunk_overlap=8 --candidate-cfg runtime.sequence_delta_pose=4`。
