...
direct benchmark CLI 现在也会把 mesh compare 相关参数收紧到真正需要它们的场景：`--mesh-chunk-size`、`--mesh-chunk-overlap`、`--mesh-support-base-dir` 都要求同时存在 `--mesh-reference`；其中 `--mesh-chunk-overlap` 还必须与 `--mesh-chunk-size` 成对出现，并要求 `--mesh-chunk-size > 0`、`--mesh-chunk-overlap >= 0`。未提供 reference、只单独传 overlap，或传入非正/负值时，这些参数都会直接报错，而不会再静默忽略。若提供了 `--mesh-reference` 但没显式传 `--mesh-support-base-dir`，CLI 才会默认回退到 `support_files`。
