...
若 official-run 段本身因为 `MoSh.prepare_cfg(...)` 配置错误、`run_moshpp_once(cfg)` 运行期文件错误、依赖链缺失而抛 `ImportError` / `ModuleNotFoundError`，或最终没有真正产出预期的 `stageii.pkl` 而失败，single runner 现在也会直接以 CLI error 退出，而不是把原始 Python 栈追踪打到终端。
