...
def _expected_stageii_path(argv):
    return argv[argv.index("--expected-stageii-path") + 1]


def _expected_benchmark_output(argv):
    return argv[argv.index("--expected-benchmark-output") + 1]


def _expected_mesh_export_paths(argv):
    return (
        argv[argv.index("--expected-mesh-obj-path") + 1],
        argv[argv.index("--expected-mesh-pc2-path") + 1],
    )
...
def test_run_stageii_torch_pair_main_runs_baseline_then_candidate_with_shared_and_side_cfgs(
    ...
    assert payload == {
        "baseline": {
            "benchmark": None,
            "stageii_path": str(tmp_path / "work" / "input" / "wolf001" / "capture_baseline_stageii.pkl"),
        },
        "candidate": {
            "benchmark": {
                "artifact": {
            ...
            "stageii_path": str(tmp_path / "work" / "input" / "wolf001" / "capture_candidate_stageii.pkl"),
        },
    }
    ...
    assert payload == {
        ...
        "candidate": {
            "benchmark": {
                "artifact": {
                    "report_path": str(
                        tmp_path / "work" / "input" / "wolf001" / "manual_candidate_candidate_benchmark.json"
                    )
                }
            },
            "stageii_path": str(
                tmp_path / "work" / "input" / "wolf001" / "manual_candidate_candidate_stageii.pkl"
            ),
        },
    }


def test_run_stageii_torch_pair_main_forwards_stageii_reference_to_benchmark_runs(
    tmp_path, monkeypatch
):
    captured = {"calls": []}
...
def test_run_stageii_torch_pair_main_can_request_baseline_benchmark_output(tmp_path, monkeypatch):
    ...
    assert payload["baseline"]["benchmark"]["artifact"]["report_path"] == str(
        tmp_path / "baseline_benchmark.json"
    )
...
def test_run_stageii_torch_pair_main_forwards_mesh_export_flags_to_both_runs(tmp_path, monkeypatch):
    ...
    assert payload["baseline"]["mesh_export"] == {
        "obj_path": str(mesh_output_dir / "capture_baseline_stageii.obj"),
        "pc2_path": str(mesh_output_dir / "capture_baseline_stageii.pc2"),
    }
    assert payload["candidate"]["mesh_export"] == {
        "obj_path": str(mesh_output_dir / "capture_candidate_stageii.obj"),
        "pc2_path": str(mesh_output_dir / "capture_candidate_stageii.pc2"),
    }
...
def test_run_stageii_torch_pair_main_rejects_non_positive_mesh_chunk_size(
    tmp_path, monkeypatch, capsys
):
    monkeypatch.setattr(
        run_stageii_torch_pair.run_stageii_torch_official,
...
def test_run_stageii_torch_pair_main_rejects_candidate_benchmark_output_colliding_with_baseline_mesh_plan(
    tmp_path, monkeypatch, capsys
):
    mesh_output_dir = tmp_path / "mesh_exports"
    baseline_stageii = tmp_path / "baseline" / "baseline_stageii.pkl"
    colliding_benchmark = mesh_output_dir / "baseline_stageii.obj"

    monkeypatch.setattr(
        run_stageii_torch_pair.run_stageii_torch_official,
        "run",
        lambda *args, **kwargs: pytest.fail(
            "underlying runner should not run when candidate benchmark output collides with baseline mesh plan"
        ),
    )
    ...
    with pytest.raises(SystemExit):
        run_stageii_torch_pair.main(
            [
                ...
                str(mesh_output_dir),
                "--baseline-cfg",
                f"dirs.stageii_fname={baseline_stageii}",
                "--candidate-benchmark-output",
                str(colliding_benchmark),
            ]
        )

    assert "candidate benchmark output collides with baseline mesh plan" in capsys.readouterr().err


def test_run_stageii_torch_pair_main_rejects_candidate_benchmark_output_colliding_with_baseline_stageii_plan(
    tmp_path, monkeypatch, capsys
):
    baseline_stageii = tmp_path / "baseline" / "baseline_stageii.pkl"
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *args, **kwargs: pytest.fail(
            "underlying runner should not run when candidate benchmark output collides with baseline stageii plan"
        ),
    )
    ...
    assert "candidate benchmark output collides with baseline stageii plan" in capsys.readouterr().err
...
def test_run_stageii_torch_pair_main_rejects_baseline_benchmark_output_colliding_with_candidate_stageii_plan(
    tmp_path, monkeypatch, capsys
):
    candidate_stageii = tmp_path / "candidate" / "shared_stageii.pkl"
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *args, **kwargs: pytest.fail(
            "underlying runner should not run when baseline benchmark output collides with candidate stageii"
        ),
    )
    ...
    with pytest.raises(SystemExit):
        run_stageii_torch_pair.main(
            [
                ...
                str(tmp_path / "work"),
                "--baseline-benchmark-output",
                str(candidate_stageii),
                "--candidate-cfg",
                f"dirs.stageii_fname={candidate_stageii}",
            ]
        )

    assert "baseline benchmark output collides with candidate stageii plan" in capsys.readouterr().err
...
def test_run_stageii_torch_pair_main_errors_when_baseline_actual_mesh_export_collides_with_candidate_benchmark_plan(
    tmp_path, monkeypatch, capsys
):
    mesh_output_dir = tmp_path / "mesh_exports"
    candidate_benchmark = tmp_path / "candidate" / "candidate_benchmark.json"
    calls = []
    ...
    def fake_run(argv, *, emit_json):
        ...
        if len(calls) == 1:
            return {
                "benchmark": None,
                "mesh_export": {
                    "obj_path": str(candidate_benchmark),
                    "pc2_path": str(mesh_output_dir / "baseline_stageii.pc2"),
                },
                "stageii_path": str(tmp_path / "baseline" / "baseline_stageii.pkl"),
            }
        pytest.fail(
            "candidate runner should not run when baseline actual mesh export collides with candidate benchmark plan"
        )
    ...
    assert len(calls) == 1
    assert "baseline actual mesh export output collides with candidate benchmark plan" in capsys.readouterr().err


def test_run_stageii_torch_pair_main_errors_when_baseline_actual_stageii_path_collides_with_candidate_benchmark_plan(
    tmp_path, monkeypatch, capsys
):
    candidate_benchmark = tmp_path / "candidate" / "candidate_benchmark.json"
    calls = []
    ...
    def fake_run(argv, *, emit_json):
        ...
        if len(calls) == 1:
            return {
                "benchmark": None,
                "stageii_path": str(candidate_benchmark),
            }
        pytest.fail(
            "candidate runner should not run when baseline actual stageii path collides with candidate benchmark plan"
        )
    ...
    assert len(calls) == 1
    assert "baseline actual stageii output path collides with candidate benchmark plan" in capsys.readouterr().err
...
def test_run_stageii_torch_pair_main_errors_when_baseline_actual_benchmark_output_collides_with_candidate_mesh_plan(
    tmp_path, monkeypatch, capsys
):
    mesh_output_dir = tmp_path / "mesh_exports"
    candidate_stageii = tmp_path / "candidate" / "shared_stageii.pkl"
    ...
    def fake_run(argv, *, emit_json):
        ...
        if len(calls) == 1:
            return {
                "benchmark": {
                    "artifact": {
                        "report_path": str(mesh_output_dir / "shared_stageii.obj"),
                    }
                },
                "mesh_export": {
                    "obj_path": _expected_mesh_export_paths(argv)[0],
...
def test_run_stageii_torch_pair_main_errors_when_candidate_actual_stageii_path_drifts_from_candidate_plan(
    ...
    calls = []
    drifted_stageii = tmp_path / "candidate" / "actual_stageii.pkl"

    def fake_run(argv, *, emit_json):
        calls.append((list(argv), emit_json))
        expected_stageii_path = argv[argv.index("--expected-stageii-path") + 1]
        if len(calls) == 1:
            return {
                "benchmark": None,
                "stageii_path": expected_stageii_path,
            }
        return {
            "benchmark": {
                "artifact": {
                    "report_path": argv[argv.index("--expected-benchmark-output") + 1],
                }
            },
            "stageii_path": str(drifted_stageii),
        }
    ...
    assert len(calls) == 2
    assert "candidate actual stageii output path drifted from candidate plan" in capsys.readouterr().err


def test_run_stageii_torch_pair_main_errors_when_candidate_actual_benchmark_output_drifts_from_candidate_plan(
    tmp_path, monkeypatch, capsys
):
    calls = []
    drifted_benchmark = tmp_path / "candidate" / "actual_benchmark.json"
    ...
    def fake_run(argv, *, emit_json):
        ...
        return {
            "benchmark": {
                "artifact": {
                    "report_path": str(drifted_benchmark),
                }
            },
            "stageii_path": expected_stageii_path,
        }
    ...
    assert len(calls) == 2
    assert "candidate actual benchmark output path drifted from candidate plan" in capsys.readouterr().err


def test_run_stageii_torch_pair_main_errors_when_candidate_actual_mesh_export_drifts_from_candidate_plan(
    tmp_path, monkeypatch, capsys
):
    calls = []
    drifted_obj_path = tmp_path / "mesh_exports" / "actual_candidate.obj"
    ...
    def fake_run(argv, *, emit_json):
        ...
        if len(calls) == 1:
            return {
                "benchmark": None,
                "mesh_export": {
                    "obj_path": argv[argv.index("--expected-mesh-obj-path") + 1],
                    "pc2_path": argv[argv.index("--expected-mesh-pc2-path") + 1],
                },
                "stageii_path": expected_stageii_path,
            }
        return {
    ...
    assert len(calls) == 2
    assert "candidate actual mesh export output path drifted from candidate plan" in capsys.readouterr().err
