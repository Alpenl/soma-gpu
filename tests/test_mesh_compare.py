...
def test_summarize_mesh_sequence_uses_explicit_chunk_keep_starts_when_present(tmp_path, monkeypatch):
    mesh_compare = _load_mesh_compare_module()
    sample_path = tmp_path / "demo_stageii.pkl"
    sample_path.write_bytes(
        pickle.dumps(
    ...
    monkeypatch.setattr(mesh_compare.render_video, "load_vertices", lambda stageii_pkl, model: expected_vertices)

    loaded = mesh_compare.load_mesh_sequence(sample_path, support_base_dir="/support-files")
    summary = mesh_compare.summarize_mesh_sequence(loaded)

    assert summary["mesh_seam_jump_l2"]["count"] == 1
    assert summary["mesh_seam_jump_l2"]["mean"] == pytest.approx(40.0)
